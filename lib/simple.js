 ...
 * Modificato da:
 * - https://github.com/troncare
 * - https://github.co/troncare/zyk-bot
*/
...
const fileTypeFromBuffer = fileType.fileTypeFromBuffer || (fileType.default && fileType.default.fileTypeFromBuffer);
...
export function makeWASocket(connectionOptions, options = {}) {
  ...
  const sock = Object.defineProperties(conn, {
    ...
    sendFile: {
       ...
       * @param {import('@realvare/baileys').proto.WebMessageInfo} quoted
       * @param {Boolean} ptt
...
sendList: {
  ...
  const sections = [...listSections]
    ...
    pin: {
       ...
       * @param {Number} time - Time in seconds for pin, 0 for unpin
       */
      async value(jid, key, time = 0) {
        const type = time > 0 ? 1 : 2;
        return await conn.sendMessage(jid, { pin: { type, key, ...(type === 1 ? { time } : {}) } });
      },
      enumerable: true,
    },
    cMod: {
       ...
       * cMod
       * @param {String} jid
       * @param {import('@realvare/baileys').proto.WebMessageInfo} message
       * @param {String} text
       * @param {String} sender
       * @param {*} options
       * @returns
    ...
    loadMessage: {
       ...
       * @returns {import('@realvare/baileys').proto.WebMessageInfo}
       */
      value(messageID) {
        if (!messageID) return null;
        for (const [chatJid, chat] of Object.entries(this.chats)) {
          if (chat.messages && chat.messages[messageID]) {
    ...
    processMessageStubType: {
       ...
       * @param {import('@realvare/baileys').proto.WebMessageInfo} m
       */
      async value(m) {
        ...
        for (const message of m) {
          try {
            ...
            if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
              /**
               * @type {import('@realvare/baileys').proto.IContextInfo}
               */
              const context = message.message[mtype].contextInfo;
              let participant = conn.decodeJid(context.participant);
              const remoteJid = conn.decodeJid(context.remoteJid || participant);
              /**
               * @type {import('@realvare/baileys').proto.IMessage}
               *
               */
...
export function smsg(conn, m, store) {
  ...
  m.reply = (text, chatId = m.chat, options = {}) => {
      return Buffer.isBuffer(text) 
        ? conn.sendFile(chatId, text, 'file', '', m, false, options) 
        : conn.sendMessage(chatId, { ...options, text }, { quoted: m, ...options })
  }
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    isGroup: {
      ...
      enumerable: true,
        ...
        sender: {
            get() {
                return this._sender || (this.conn?.decodeJid(this.key?.fromMe && this.conn?.user.id || this.participant || this.key.participant || this.chat || ''))
            },
            set(value) {
                this._sender = value;
            },
    ...
    quoted: {
      get() {
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {text: q} : q)), {
          ...
          sender: {
            get() {
              return this._sender || (contextInfo.participant || this.chat || '').decodeJid();
            },
            set(value) {
              this._sender = value;
            },
          ...
          value(saveToFile = false) {
            ...
            if (!mtype || !this.mediaMessage) {
                console.error('[Download Error] No media message found')
